...
const veganos = foodList1.filter(plato => plato.isVeggie == true).map((vegano) => {
    if (vegano.name === "Tempeh"){
      return `Que rico ${vegano.name} me voy a comer!`
    }else if (vegano.name === "Tofu burguer")
      return `Que rica ${vegano.name} me voy a comer!`  
  })
console.log (veganos)
...
const frases1 = staff.map(persona => `${persona.name} es ${persona.role} y le gusta ${persona.hobbies[0]} y ${persona.hobbies[1]}`)
...
const caros = inventory.filter(item => item.price > 300).map(caro => caro.name)
console.log(caros)
